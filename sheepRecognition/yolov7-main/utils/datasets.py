...
class LoadStreams:  # multiple IP or RTSP cameras
    def __init__(self, sources='streams.txt', img_size=640, stride=32):
        ...
        print("Chose Load Stream")

        if False: #os.path.isfile(sources):
        ...
        n = len(sources)
        print(f'Original sources: {sources}')
        self.imgs = [None] * n
        self.sources = [clean_str(x) for x in sources]  # clean source names for later
        print(f'Proccessed string: {self.sources}')
        for i, s in enumerate(sources):
            # Start the thread to read frames from the video stream
            print(f'source {i + 1} over {n} sources: {s}... ', end='')
            ...
            w = int(cap.getW())
            h = int(cap.getH())
            self.fps = cap.getFPS() % 100

            print(f'Camera successfully opened with Width: {w}, Height: {h}, FPS: {self.fps}')
